...
getRef('send_update_popup').addEventListener('option-clicked', e => {
    console.log(e)
})
        ...
        if (floGlobals.isUserLoggedIn) {
            if (floGlobals.isSubAdmin) {
                actions = html`
                    ...
                    <a href=${`#/task?id=${id}`} class="button button--outlined margin-left-auto">${floGlobals.applications[id]?.size || 0} working</a>
                `
